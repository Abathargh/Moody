...
def windowBuilder( audio_binary_string ):

    audio_list = np.frombuffer( audio_binary_string, dtype = np.int32 )
    counter = 0
    chunk_list = list()
    chunk = None
    window = ChunkWindow()
    ...
    for frame in audio_list:
        if not (counter == 0) and counter % 8192 == 0:
            chunk = AudioChunk( b"".join( chunk_list ), pyaudio.paInt32 )
            window.add(chunk)

        chunk_list.add( frame )
        counter += 1
